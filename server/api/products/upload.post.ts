...
export default defineEventHandler(async (event) => {
  ...
  const { productId, seo_title, meta_description, short_description, html_content, keywords, images, price, categories, attributes } = body
  ...
  try {
    ...
    // Upload to WooCommerce with categories and attributes
    const wooRepo = new WooCommerceRepository()
    const { wooCommerceId, previewUrl } = await wooRepo.uploadProduct(updatedProduct, categories, attributes)
